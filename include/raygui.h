...
static int GetTextHeight(const char *text);                    // Gui get text width using default font
...
void GuiLabelEx(Rectangle bounds, const char *text, Color col) {
    GuiControlState state = guiState;
...
bool GuiCheckBox(Rectangle bounds, const char *text, bool checked) {
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked) {
        ...
        Rectangle totalBounds = {
            ...
            bounds.y - 10,
            bounds.width + textBounds.width + GuiGetStyle(CHECKBOX, TEXT_PADDING) + 5,
            bounds.height + 5,
        };
...
static int GetTextWidth(const char *text, int sizex) {
    Vector2 size = {0};
